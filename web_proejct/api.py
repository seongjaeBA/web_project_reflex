...
async def get_product(spec_id: int):
    ...
    with rx.session() as session:
        spec = session.get(Movie, spec_id)
...
async def list_product(req: Request):
    ...
    with rx.session() as session:
        specs = session.scalars(select(Movie)).all()
    return specs
...
async def add_product(req: Request):
    ...
    with rx.session() as session:
        ...
        code = data["MovieCode"]
        if not code:
            return HTTPException(status_code=402, detail="Invalid `MovieCode`")
        session.add(
            Movie(
                MovieCode= code,
                created=now,
...
product_router = APIRouter(prefix="/Movie", tags=["Movie"])
