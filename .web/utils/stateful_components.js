...
export function Hstack_ab4794008275880551112d3b8dc3895c () {
  const state = useContext(StateContexts.state)


  return (
    <HStack sx={{"bg": isTrue((state.router.page.path === "/treatment generate") || (((state.router.page.path === "/") && "Treatment Generate") === "Home")) ? `#F5EFFE` : `transparent`, "color": isTrue((state.router.page.path === "/treatment generate") || (((state.router.page.path === "/") && "Treatment Generate") === "Home")) ? `#1A1060` : `black`, "borderRadius": "0.375rem", "boxShadow": "0px 0px 0px 1px rgba(84, 82, 95, 0.14)", "width": "100%", "paddingX": "1em"}}>
  <ChakraImage src={`/github.svg`} sx={{"height": "2.5em", "padding": "0.5em"}}/>
  <Text>
  {`Treatment Generate`}
</Text>
